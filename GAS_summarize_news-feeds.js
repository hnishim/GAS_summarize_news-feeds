...
1. 以下の対象に合致するかを判定してください
  - 判定は厳しく行ってください。偽陰性（False negative）よりも偽陽性（False Positive）を避けてください。完全対象記事を1.0, 完全対象外記事を0.0とした場合、閾値は0.8に設定し、0.8未満の記事は対象外と判定してください。
2. 判定結果をもとに、以下のように対応してください
  - 条件に合致した場合：日本語で200字以内で、余計な枕詞は追加せず、記事の内容だけから要約を生成し、要約した文章のみを出力してください。（あなたからの出力内容が直接Slackを経由して複数のメンバーに共有されます。余計な文言を追加することは絶対にしないでください。）
  - 条件に合致しなかった場合：「Z」とだけ出力してください（「Z」の前後に何か余計な文字があった場合、あなたからの出力内容が直接Slackを経由して複数のメンバーに共有されます。かならず「Z」の1文字だけを出力してください。）
  - 対象の記事がないなど、条件判定ができなかった場合：「Z」とだけ出力してください（「Z」の前後に何か余計な文字があった場合、あなたからの出力内容が直接Slackを経由して複数のメンバーに共有されます。かならず「Z」の1文字だけを出力してください。）
3. 出力予定の文章全体をもう一度、本手順に厳格にしたがっているか確認してください。問題がなければそのまま出力し、処理を完了してください。
  - たとえば、出力予定の文章が300文字以上ある場合は明らかに手順に沿っていません。もう一度手順に沿って再生成してください。

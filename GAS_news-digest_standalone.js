...
function bootstrapRssState() {
  ...
  try {
    ...
    feedResults.forEach(result => {
      ...
      const ids = selectRssWindow(result.items).map(item => item.id);
...
function processRssNews(pipeline) {
  ...
  feedResults.forEach(result => {
    ...
    if (!stateStore.isInitialized(source.id)) {
      stateStore.initialize(source.id, selectRssWindow(result.items).map(item => item.id));
      Logger.log(`[${source.name}] 初回取得のため既存記事を処理済みとして登録しました。`);
      return;
    }

    const unseenItems = selectRssWindow(result.items)
...
function processUnreadNewsletters(pipeline) {
  ...
  messages.forEach(message => {
    try {
      ...
      // ログ取得が失敗すると後続メールまで止まるため、単件情報の再取得はしない。
      Logger.log(`メール処理失敗: ${error}`);
    }
  });
}
...
function fetchAllRssFeeds() {
  ...
  try {
    ...
    Logger.log(`RSS一括取得失敗。source別に再取得します: ${error}`);
    return requests.map((request, index) => {
      try {
        return parseRssResponse(UrlFetchApp.fetch(request), sources[index]);
      } catch (sourceError) {
        return {
  ...
  return sources.map((source, index) => parseRssResponse(responses[index], source));
}

function parseRssResponse(response, source) {
  let responseCode = null;
  try {
    responseCode = response.getResponseCode();
    if (responseCode < 200 || responseCode >= 300) {
      return { source, items: [], responseCode, error: `HTTP ${responseCode}` };
    }
    return {
      source,
    ...
    return { source, items: [], responseCode, error: String(error) };
  }
}
...
function parseFeed(xmlText, source) {
  ...
  const rootNamespace = root.getNamespace().getURI();
  let entries;

  if (rootName === 'rss' && rootNamespace === '') {
    ...
    throw new Error(`未知のXML feed形式: ${root.getName()} (${rootNamespace})`);
  }
...
/**
 * 有限の既読ID履歴に対応する、最新記事だけの処理対象窓。
 * 古い記事を履歴から落とした後に再投稿しないため、未処理判定より前に絞る。
 */
function selectRssWindow(items) {
  return items.slice()
    .sort((left, right) => compareItemsOldestFirst(right, left))
    .slice(0, NEWS_DIGEST_CONFIG.RSS.STORED_IDS_PER_SOURCE);
}
